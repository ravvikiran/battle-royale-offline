## Victory screen displayed when the player wins a match (placement == 1).
...
func _display_results() -> void:
	...
	var placement: int = match_result.get("placement", 1)
	...
	# Placement always shows the "#n / total" form (the "/ total" is static context).
	...
	# Reveal order reinforces hierarchy: placement lands first, then the
	# performance cards cascade + count up.
